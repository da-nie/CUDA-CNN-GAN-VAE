...
static const uint32_t TGA_DESCRIPTOR_ALPHA_MASK=(1<<3)|(1<<2)|(1<<1)|(1<<0);//разрядность альфа-канала
static const uint32_t TGA_DESCRIPTOR_UP_TO_DOWN_MASK=(1<<5);//сверху-вниз
static const uint32_t TGA_DESCRIPTOR_RIGHT_TO_LEFT_MASK=(1<<4);//справа-налево
static const uint32_t TGA_DESCRIPTOR_ALPHA_8_BIT=8;//разpядность альфа-канала
...
static uint8_t *GetTGAImage(uint8_t *data_ptr,uint32_t length,int32_t &width,int32_t &height)
{
 ...
 if (sTGAHeader.descriptor&TGA_DESCRIPTOR_RIGHT_TO_LEFT_MASK) return(NULL);//справа-налево не поддерживаем
 ...
 if (sTGAHeader.bits==24)//BGR - модицифируем для четвёрок байт
 {
  ...
  int32_t y,x;
  if (sTGAHeader.descriptor&TGA_DESCRIPTOR_UP_TO_DOWN_MASK)//прямой формат
  {
  ...
  if ((sTGAHeader.descriptor&TGA_DESCRIPTOR_UP_TO_DOWN_MASK)==0)//обратный формат
  {
   uint8_t *oi_ptr=out_image;
   uint8_t *i_ptr=data_ptr+image_offset+sTGAHeader.width*(sTGAHeader.height-1)*3;
 ...
 if (sTGAHeader.bits==32)//BGR - модицифируем для четвёрок байт
 {
  ...
  {
   uint8_t *oi_ptr=out_image;
   uint8_t *i_ptr=data_ptr+image_offset+sTGAHeader.width*(sTGAHeader.height-1)*4;
 ...
 if (sTGAHeader.colorMapType==TGA_COLOR_MAP_PALETTE && sTGAHeader.colorMapBits/8==3)//есть палитра по 24 бита (другую палитру не поддерживаем)
 {
  ...
  if ((sTGAHeader.descriptor&TGA_DESCRIPTOR_UP_TO_DOWN_MASK)==0)//формат перевёрнутый
  {
   uint8_t *oi_ptr=out_image;
...
bool SaveTGA(const char *file_name,int32_t width,int32_t height,uint8_t *image)
{
 ...
 sTGAHeader.imageType=TGA_IMAGE_TYPE_RGB;
 sTGAHeader.colorMapStart=0;
 sTGAHeader.colorMapLength=0;
 sTGAHeader.colorMapBits=0;
 sTGAHeader.xstart=0;
 sTGAHeader.ystart=0;
 sTGAHeader.width=static_cast<uint16_t>(width);
 sTGAHeader.height=static_cast<uint16_t>(height);
 sTGAHeader.bits=32;
 sTGAHeader.descriptor=TGA_DESCRIPTOR_UP_TO_DOWN_MASK|TGA_DESCRIPTOR_ALPHA_8_BIT;
 ...
 if (cRAIIFileOut.GetHandle().write(reinterpret_cast<char*>(image),sizeof(uint8_t)*width*height*sizeof(uint32_t)).fail()==true) return(false);
 return(true);
}
