...
//----------------------------------------------------------------------------------------------------
template<class type_t>
void CNetLayerAveragePooling<type_t>::EnableEMA(bool state)
{
 EMAEnabled=state;
}
//----------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------
template<class type_t>
bool CNetLayerAveragePooling<type_t>::LoadEMAWeight(IDataStream *iDataStream_Ptr,bool check_size)
{
 if (check_size==true)
 {
  if (AveragePooling_X!=iDataStream_Ptr->LoadUInt32()) throw("Ошибка загрузки слоя сжатия с усреднением: неверный коэффициент масштабирования по X.");
...
//----------------------------------------------------------------------------------------------------
template<class type_t>
bool CNetLayerAveragePooling<type_t>::SaveEMAWeight(IDataStream *iDataStream_Ptr)
{
 iDataStream_Ptr->SaveUInt32(AveragePooling_X);
 iDataStream_Ptr->SaveUInt32(AveragePooling_Y);
 return(true);
