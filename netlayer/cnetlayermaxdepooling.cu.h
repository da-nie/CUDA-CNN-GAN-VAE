...
//----------------------------------------------------------------------------------------------------
/*!создать слой
\param[in] depooling_y Коэффициент расжатия по Y
\param[in] depooling_x Коэффициент расжатия по X
...
void CNetLayerMaxDePooling<type_t>::SetOutput(CTensor<type_t> &output)
{
 ...
 if (output.GetSizeW()!=cTensor_H.GetSizeW()) throw("void CNetLayerMaxDePooling<type_t>::SetOutput(CTensor<type_t> &output) - ошибка размерности тензора output!");
 cTensor_H=output;
}
//----------------------------------------------------------------------------------------------------
/*!задать выход слоя
...
void CNetLayerMaxDePooling<type_t>::GetOutput(CTensor<type_t> &output)
{
 ...
 if (output.GetSizeW()!=cTensor_H.GetSizeW()) throw("void CNetLayerMaxDePooling<type_t>::GetOutput(CTensor<type_t> &output) - ошибка размерности тензора output!");
 output=cTensor_H;
}
//----------------------------------------------------------------------------------------------------
  ...
  for(uint32_t z=0;z<input_z;z++)
  {
   for(uint32_t y=0;y<input_y;y++)
   {
    for(uint32_t x=0;x<input_x;x++)
    {
     ...
     res/=divider;
     cTensor_PrevLayerError.SetElement(w,z,y,x,res);
    }
   }
  }
 }
