...
void CNetLayerConvolution<type_t>::Create(uint32_t kernel_amount,uint32_t kernel_size,int32_t stride_x,int32_t stride_y,int32_t padding_x,int32_t padding_y,INetLayer<type_t> *prev_layer_ptr,uint32_t batch_size)
{
 ...
 cTensor_Kernel=CTensor<type_t>(1,1,Kernel_Amount,Kernel_Z*Kernel_X*Kernel_Y);
...
void CNetLayerConvolution<type_t>::GetOutput(CTensor<type_t> &output)
{
 ...
 if (output.GetSizeW()!=cTensor_H.GetSizeW()) throw("void CNetLayerConvolution<type_t>::GetOutput(CTensor<type_t> &output) - ошибка размерности тензора output!");
 output=cTensor_H;
}
//----------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------
template<class type_t>
void CNetLayerConvolution<type_t>::SetTimeStep(uint32_t index,uint32_t time_step)
{
}
...
//----------------------------------------------------------------------------------------------------
template<class type_t>
void CNetLayerConvolution<type_t>::PrintInputTensorSize(const std::string &name)
{
 if (PrevLayerPtr!=NULL) PrevLayerPtr->GetOutputTensor().Print(name+" Convolution: input",false);
}
//----------------------------------------------------------------------------------------------------
