...
void CNetLayerMaxPooling<type_t>::Create(uint32_t pooling_y,uint32_t pooling_x,INetLayer<type_t> *prev_layer_ptr,uint32_t batch_size)
{
 ...
 cTensor_P=CTensor<typename CTensorMath<type_t>::SPos>(BatchSize,output_z,output_y,output_x);
...
void CNetLayerMaxPooling<type_t>::SetOutput(CTensor<type_t> &output)
{
 ...
 if (output.GetSizeW()!=cTensor_H.GetSizeW()) throw("void CNetLayerMaxPooling<type_t>::SetOutput(CTensor<type_t> &output) - ошибка размерности тензора output!");
 cTensor_H=output;
}
//----------------------------------------------------------------------------------------------------
/*!задать выход слоя
...
void CNetLayerMaxPooling<type_t>::GetOutput(CTensor<type_t> &output)
{
 ...
 if (output.GetSizeW()!=cTensor_H.GetSizeW()) throw("void CNetLayerMaxPooling<type_t>::GetOutput(CTensor<type_t> &output) - ошибка размерности тензора output!");
 output=cTensor_H;
}
//----------------------------------------------------------------------------------------------------
...
void CNetLayerMaxPooling<type_t>::TrainingBackward(bool create_delta_weight)
{
 ...
 CTensorMath<type_t>::MaxPoolingBackward(cTensor_PrevLayerError,cTensor_P,cTensor_Delta,Pooling_X,Pooling_Y);
...
//----------------------------------------------------------------------------------------------------
template<class type_t>
void CNetLayerMaxPooling<type_t>::SetTimeStep(uint32_t index,uint32_t time_step)
{
}
...
//----------------------------------------------------------------------------------------------------
template<class type_t>
void CNetLayerMaxPooling<type_t>::PrintInputTensorSize(const std::string &name)
{
 if (PrevLayerPtr!=NULL) PrevLayerPtr->GetOutputTensor().Print(name+" MaxPooling: input",false);
}
//----------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------
template<class type_t>
void CNetLayerMaxPooling<type_t>::PrintOutputTensorSize(const std::string &name)
{
 GetOutputTensor().Print(name+" MaxPooling: output",false);
}
//----------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------
template<class type_t>
void CNetLayerMaxPooling<type_t>::EnableEMA(bool state)
{
 EMAEnabled=state;
}
//----------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------
template<class type_t>
bool CNetLayerMaxPooling<type_t>::LoadEMAWeight(IDataStream *iDataStream_Ptr,bool check_size)
{
 if (check_size==true)
 {
  if (Pooling_X!=iDataStream_Ptr->LoadUInt32()) throw("Ошибка загрузки слоя уменьшения максимумом: неверный коэффициент масштабирования по X.");
...
//----------------------------------------------------------------------------------------------------
template<class type_t>
bool CNetLayerMaxPooling<type_t>::SaveEMAWeight(IDataStream *iDataStream_Ptr)
{
 iDataStream_Ptr->SaveUInt32(Pooling_X);
 iDataStream_Ptr->SaveUInt32(Pooling_Y);
 return(true);
